-- Filter out short chunks (< 100 chars) from search results
-- These are typically case numbers, headers, or metadata that produce poor search matches
-- Minimum chunk length of 100 characters ensures meaningful content is being matched
...
-- =========================
-- CASES: SEMANTIC SEARCH (WITH MIN CHUNK LENGTH)
-- =========================
...
-- =========================
-- CASES: HYBRID SEARCH (WITH MIN CHUNK LENGTH)
-- =========================
...
BEGIN
    ...
    keyword_matches AS (
        SELECT 
            cc.id AS case_id,
            ts_rank_cd(cc.search_vector, ts_query)::FLOAT AS kw_score
        FROM court_cases cc
        WHERE cc.search_vector @@ ts_query
        LIMIT match_count * 10
    ),
    combined AS (
        SELECT 
...
-- =========================
-- LEGISLATION: SEMANTIC SEARCH (WITH MIN CHUNK LENGTH)
-- =========================
...
-- =========================
-- LEGISLATION: HYBRID SEARCH (WITH MIN CHUNK LENGTH)
-- =========================
...
BEGIN
    ...
    keyword_matches AS (
        SELECT 
            l.id AS leg_id,
            ts_rank_cd(l.search_vector, ts_query)::FLOAT AS kw_score
        FROM legislation l
