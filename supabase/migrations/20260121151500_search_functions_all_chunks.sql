-- Improve search quality by considering all chunks instead of only chunk_index = 0
-- This replaces the previous functions with versions that aggregate nearest
-- neighbors across all chunks per document, then pick the best chunk per doc.
