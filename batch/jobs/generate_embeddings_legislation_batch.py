"""Generate embeddings for legislation_sections using AWS Bedrock Batch API.
...
from pipeline.chunking import chunk_legislation_section
...
async def export_sections_to_jsonl(
    output_file: Path,
    limit: Optional[int] = None,
    max_records_per_file: int = 50000,
) -> int:
    """Export legislation sections without embeddings to JSONL format for Bedrock Batch.
    ...
    try:
        ...
        if not rows:
            log.info("No sections found that require embeddings")
            return 0
        
        log.info("Fetched sections", count=len(rows))
        ...
        for row in rows:
            ...
            chunks = chunk_legislation_section(section_id, content, section_path=section_path)
            ...
            for chunk in chunks:
                ...
                record_id = f"section-{section_id}-chunk-{chunk.chunk_index}"
                record = {
                    "recordId": record_id,
                    "modelInput": {
            ...
            if (rows.index(row) + 1) % 100 == 0:
                log.info("Progress", processed_sections=rows.index(row) + 1, total_records=record_count)
...
def submit_batch_job(input_s3_uri: str, output_s3_prefix: str) -> BatchJobInfo:
    ...
    job_name = f"legal-ai-legislation-embeddings-{timestamp}"
        ...
        for output_file in output_files:
            ...
            with output_file.open("r") as f:
                for line in f:
                    ...
                    # Parse recordId: "section-{section_id}-chunk-{chunk_index}"
                    # Note: section_id is a UUID with hyphens, so we need to parse more carefully
                    record_id = record["recordId"]
                    
                    if not record_id.startswith("section-"):
                        log.warning("Invalid recordId format - missing 'section-' prefix", record_id=record_id)
                        continue
                    
                    # Remove "section-" prefix
                    remainder = record_id[8:]
                    ...
                    section_id = remainder[:chunk_pos]
                    ...
                    sql = """
                    INSERT INTO legislation_embeddings_cohere (section_id, chunk_index, chunk_type, chunk_text, embedding)
                    VALUES ($1, $2, $3, $4, $5)
                    ...
                    await conn.execute(
                        ...
                        section_id,
                        chunk_index,
                        "section_body",  # chunk_type for legislation
...
async def run_export_step(limit: Optional[int] = None) -> list[str]:
    """Run export step for legislation: generate JSONL file(s) and upload to S3.
    ...
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    base_file = output_dir / f"legislation-embeddings-batch-{timestamp}.jsonl"
    
    # Export sections to JSONL (may create multiple files)
    record_count = await export_sections_to_jsonl(base_file, limit=limit)
    ...
    pattern = f"legislation-embeddings-batch-{timestamp}*.jsonl"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate legislation embeddings using Bedrock Batch API")
